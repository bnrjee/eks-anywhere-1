...
func autofill(ctx context.Context) error {
	...
	for _, workerNodeGroupConfiguration := range machineConfig[clusterConfig.Spec.WorkerNodeGroupConfigurations {
		updateFieldInt("WORKER_NODE_COUNT", workerNodeGroupConfiguration.Count)
		workerMachineConfig := workerNodeGroupConfigurations.MachineGroupRef.Name]
